...
1. `/trpg帮助` — 指令总表（**不要用 `/help`**；本指令强制纯文本输出）
2. `/trpgquick 记者` — 随机属性、默认技能、设为当前角色  
3. `/trpgpc` — 看角色卡（图卡）  
4. `/trpgroll` — 默认 `1d100`，返回**拟真骰子图 + 文字**  
5. `/trpgcheck 50` — 对照 50% 技能检定  
6. `/trpgsan` — 用当前角色 SAN 做理智检定并写回  

---

## 指令手册

除 `/trpg帮助`、`/trpg设置` 外，指令名均为 **英文 + `trpg` 前缀**，避免与其它插件冲突。  
下表中 `b` = 奖励骰，`p` = 惩罚骰，可连写（`bb`、`bp`）。  
除纯掷骰外，涉及角色的指令默认操作 **当前角色**；可先 `/trpguse 名` 切换。

### 掷骰与检定

| 指令 | 参数 | 说明 |
|------|------|------|
| `/trpgroll` | `[表达式] [b\|p]…` | 掷骰。缺省 `1d100`。表达式：`2d6`、`1d20+3` 等。仅对 `1d100` / `d100` 识别奖惩骰。输出：拟真骰子图 + 文字结果 |
| `/trpgcheck` | `技能值\|技能名 [n\|h\|e\|c] [b\|p]…` | 技能检定。难度：`n` 常规（默认）、`h`/`e` 困难（≤半）、`c` 大成功。图 + 文 |
| `/trpgsan` | `[当前SAN] [成功损失上限]` | SAN 检定。省略 SAN 时用当前角色 SAN；与之一致时检定后写回。图 + 文 |
...
| `/trpgquick` | `[职业]` | 快速车卡：随机属性 + 派生值 + 默认技能，设为当前角色 |
| `/trpgcc` | | 开始引导车卡；`/trpgcc cancel` 取消 |
| `/trpgpc` | `[角色名]` | 查看角色卡（图卡优先）。无参 = 当前角色 |
| `/trpgpcs` | | 列出本人全部角色（`*` 为当前） |
| `/trpguse` | `角色名` | 切换当前角色 |
| `/trpgdel` | `角色名` `!` | 删除角色。需带 `!` 确认 |
...
| 指令 | 参数 | 说明 |
|------|------|------|
| `/trpginit` | `名字 分数或表达式 …` | 追加并排序。分数可为 `80` 或 `1d100` |
| `/trpginit` | | 显示当前战斗轮 |
| `/trpginit` | `clear` | 清空 |

```text
/trpginit 张三 1d100 李四 72 深潜者 1d100
/trpginit clear
```
...
| 指令 | 参数 | 说明 |
|------|------|------|
| `/trpgnpcc` | `名字` | 将 **当前角色** 复制为全局 NPC 模板 |
| `/trpgnpcs` | | 列出 NPC 名 |
| `/trpgnpr` | `名字` | 读取 NPC（图卡优先） |
| `/trpgnpcd` | `名字` | 删除 NPC |
