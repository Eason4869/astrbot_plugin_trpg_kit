...
1. `/cq 记者` — 随机属性、写入默认技能、设为当前角色  
2. `/pc` — 看角色卡（图卡）  
3. `/r` — 默认 `1d100`  
4. `/ra 50` — 对照 50% 技能检定  
5. `/sc` — 用当前角色 SAN 做理智检定并写回  
...
除纯掷骰外，涉及角色的指令默认操作 **当前角色**；可先 `/use 名` 切换。

### 掷骰与检定

| 指令 | 参数 | 说明 |
|------|------|------|
| `/r` | `[表达式] [b\|p]…` | 掷骰。缺省 `1d100`。表达式：`2d6`、`1d20+3`、`2d6+1d8-2` 等。仅对 `1d100` / `d100` 识别奖惩骰 |
| `/ra` | `技能值\|技能名 [n\|h\|e\|c] [b\|p]…` | 技能检定。技能名须已在当前角色中；属性名（`STR` 等）也可。难度：`n` 常规（默认）、`h`/`e` 困难（≤半）、`c` 大成功 |
| `/sc` | `[当前SAN] [成功损失上限]` | SAN 检定。省略 SAN 时用当前角色 `derived.SAN`。有当前角色且 SAN 与之一致时，检定后写回新 SAN |
...
| 指令 | 参数 | 说明 |
|------|------|------|
| `/set` | `字段=值 …` | 批量写入。八维属性会重算派生值；同一次里写的 `SAN`/`HP`/`MP`/`luck` 在重算之后应用，不会被冲掉。技能名任意中文键 |
| `/cq` | `[职业]` | 快速车卡：随机属性 + 派生值 + 默认技能，设为当前角色 |
| `/cc` | | 开始引导车卡。随后用纯文本回复推进；`/cc cancel` 取消 |
| `/pc` | `[角色名]` | 查看角色卡（图卡优先）。无参 = 当前角色，并设为当前 |
| `/pcs` | | 列出本人全部角色（`*` 为当前） |
| `/use` | `角色名` | 切换当前角色 |
| `/del` | `角色名` `!` | 删除角色。需带 `!` 确认；删除当前角色会清空指针 |

**引导车卡流程（`/cc`）**

1. 回复角色名  
2. 回复职业，可附属性生成法：`记者 3d6` 或 `记者 2d6`（或只发 `3d6` / `2d6`）  
...
| 指令 | 参数 | 说明 |
|------|------|------|
| `/init` | `名字 分数或表达式 …` | 追加并排序。分数可为 `80` 或 `1d100` |
| `/init` | | 显示当前战斗轮 |
| `/init` | `clear` | 清空 |

```text
/init 张三 1d100 李四 72 深潜者 1d100
/init clear
```
